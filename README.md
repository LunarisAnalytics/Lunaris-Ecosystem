...
```mermaid
flowchart TD
    A[Market and On-Chain Data] --> B[Ingestion Layer]
    B --> C[Normalization and Enrichment]
    C --> D[Signal Engine]
    D --> E[Token Scoring]
    D --> F[Wallet Scoring]
    D --> G[Portfolio Context]
    E --> H[Analytics Copilot]
    F --> H
    G --> H
    D --> I[Research Agent]
    H --> J[Web Terminal]
    H --> K[Telegram Mini App]
    H --> L[Browser Companion]
    I --> J
    I --> K
    D --> M[API and SDK]
    M --> N[Runs and Jobs]
...
```mermaid
flowchart LR
    A[Blocks • Transfers • Swaps • Liquidity Events] --> B[Ingestion]
    C[Prices • Volume • Route Data] --> B
    B --> D[Normalization Layer]
    D --> E[Derived Metrics]
    E --> F[Signals]
    F --> G[Scores and Classifiers]
    G --> H[Token Views]
    G --> I[Wallet Views]
    G --> J[Portfolio Context]
    H --> K[Analytics Copilot]
    I --> K
    J --> K
    G --> L[Research Agent]
    K --> M[UI Surfaces]
...
flowchart TB
    ...
    subgraph S2[Shared Workspace]
        E[Project Account]
        F[Watchlists]
        G[History]
        H[Alerts]
        I[Credits and Limits]
        J[Saved Context]
    end

    subgraph S3[Intelligence Layer]
        K[Token Analytics]
        L[Wallet Analytics]
        M[Portfolio Context]
        N[Analytics Copilot]
        O[Research Agent]
        P[Future Execution Agent]
    end

    subgraph S4[Infrastructure]
        Q[Runs]
        R[Jobs]
        S[Webhooks]
        T[Event Streams]
